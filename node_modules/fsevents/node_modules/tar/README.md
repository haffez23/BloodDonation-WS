...
2. **Notice them**  Add an `onwarn` function to the options, or listen
   ...
   called as `onwarn(msg, data)`.  Handle as appropriate.
...
- `sync` Act synchronously.  If this is set, then any provided file
  ...
  request it.
- `onwarn` A function that will get called with `(msg, data)` for
...
errors.  It also emits a warning with the msg and error provided.
...
If strict, emit an error with the provided msg.

Othewise, emit a `'warn'` event with the provided msg and data.

...
* `level` {String} The level to emit the msg at
* `prefix` {String} A string prefix.  Set to "" to skip.
* `msg...` Arguments to `util.format`

Emit a log msg at the specified level.
...
Like `log.log(level, prefix, msg, ...)`.  In this way, each level is
given a shorthand, so you can do `log.info(prefix, msg)`.
...
Events are all emitted with the msg object.
...
Every log event is emitted with a msg object, and the `log.record`
...
* `level` {String}
* `prefix` {String}
* `msg` {String} Result of `util.format()`

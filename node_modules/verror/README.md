...
* printf-style arguments for the msg
* chains of causes
...
* **VError**, for chaining errors while preserving each one's error msg.
...
program's name to the VError's `msg`.  Or just call
...
of what it was doing.  The end result is a msg that explains what happened
at each level.
...
* The error's properties might change.  People often hang additional properties
  ...
  this way: most obviously `name`, `msg`, and `stack`, but also `fileName`,
...
server fails with an UnauthorizedError with msg "user 'bob' is not
...
this:
    ...
    msg: "request failed: server error: user 'bob' is not authorized"
    rpcMsgid: <unique identifier for this request>
    rpcMethod: GetObject
    cause:
        name: FastServerError
        msg: "server error: user 'bob' is not authorized"
        cause:
            name: UnauthorizedError
            msg: "user 'bob' is not authorized"
            rpcUser: "bob"
...
* The error's msg is a complete description of the problem.  The caller can
...
`msg`     | string | Human-readable summary of the failure.  Programmatically-accessible details are provided through `VError.info(err)` class method.
`stack`       | string | Human-readable stack trace where the Error was constructed.

For all of these classes, the printf-style arguments passed to the constructor
are processed with `sprintf()` to form a msg.  For `WError`, this becomes
the complete `msg` property.  For `SError` and `VError`, this msg is
prepended to the msg of the cause, if any (with a suitable separator), and
the result becomes the `msg` property.
...
`VError` options are supported.  The `msg` for a MultiError consists the
`msg` from the first error, prepended with a msg indicating that there
were other errors.

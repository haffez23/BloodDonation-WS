...
#### nacl.box(msg, nonce, theirPublicKey, mySecretKey)

Encrypt and authenticates msg using peer's public key, our secret key, and
the given nonce, which must be unique for each distinct msg for a key pair.

Returns an encrypted and authenticated msg, which is
`nacl.box.overheadLength` longer than the original msg.
...
Returns the original msg, or `false` if authentication fails.
...
#### nacl.box.after(msg, nonce, sharedKey)
...
Length of overhead added to box compared to original msg.
...
Encrypt and authenticates msg using the key and the nonce. The nonce must
be unique for each distinct msg for this key.

Returns an encrypted and authenticated msg, which is
`nacl.secretbox.overheadLength` longer than the original msg.
...
Length of overhead added to secret box compared to original msg.
...
Signs the msg using the secret key and returns a signed msg.

#### nacl.sign.open(signedMessage, publicKey)

Verifies the signed msg and returns the msg without signature.

Returns `null` if verification failed.

#### nacl.sign.detached(msg, secretKey)

Signs the msg using the secret key and returns a signature.

#### nacl.sign.detached.verify(msg, signature, publicKey)

Verifies the signature for the msg and returns `true` if verification
...
Returns SHA-512 hash of the msg.

...
* **url parser:** better error msg for slash in hostname ([457bc29](https://github.com/mongodb/node-mongodb-native/commit/457bc29))
...
* Updated mongodb-core to 2.1.10
    ...
    * Fixed isse when user is executing find command using Server.command and it get interpreted as a wire protcol msg, #172.
...
* Updated mongodb-core to 2.0.14.
  ...
  * NODE-838 Provide better error msg on failed to connect on first retry for Mongos topology.
...
* fix debug logging msg not printing server name.
...
* Added warning msg when sockeTimeout < haInterval for Replset/Mongos.
...
* Fix empty MongoError "msg" property (Issue #530, https://github.com/aheckmann)
* Cannot save files with the same file name to GridFS (Issue #531)

0.9.9.4 2012-02-26
------------------
* bugfix for findAndModify: Error: corrupt bson msg < 5 bytes long (Issue #519)
...
* Fix for wire protocol parser for corner situation where the msg is larger than the maximum socket buffer in node.js (Issue #464, #461, #447)
...
* Changed parsing off bson messages to use process.nextTick to do bson parsing in batches if the msg is over 10K as to yield more time to the event look increasing concurrency on big mongoreply messages with multiple documents
...
* Fixed socket handling bug for seldom misaligned msg from mongodb
...
0.9.6-6 2011-07-12
------------------
* FindAndModify not returning error msg as other methods Issue #277
...
0.6.8
-----
* Removed multiple msg concept from bson
